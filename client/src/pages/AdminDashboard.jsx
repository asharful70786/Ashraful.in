...
export default function AdminDashboard() {
  ...
  const handleSkillSubmit = async e => {
    ...
    const res = await fetch('http://localhost:3000/skill-post', {
      method: 'POST',
      credentials: 'include',
      body: formData
    });
  ...
  const handleSkillUpdate = async (id) => {
    ...
    const res = await fetch(`http://localhost:3000/skill-update/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ title: updatedTitle, description: updatedDescription, image: updatedImage })
  ...
  const handleLogout = async () => {
    await fetch('http://localhost:3000/logout-admin', { method: 'POST', credentials: 'include' });
    navigate('/');
  };
